...
const HomeLandlord = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            // Step 1: Geocoding using Mapbox API
            ...
            // Check if geocoding response has features
            if (mapboxResponse.data.features && mapboxResponse.data.features.length > 0) {
                ...
                console.log('Geocoded coordinates:', location2);

                // Update location state immediately
                setLocation(location2);

                // Step 2: Fetch nearby posts using authenticated API
                try {
                    const postsResponse = await authApi().get(endpoints.tenantPost, {
                        params: {
                            lat: location2[1], // Use updated latitude
                            lng: location2[0],
                            radius: radius, // Use updated longitude
                        }
                    });
